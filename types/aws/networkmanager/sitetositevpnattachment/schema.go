...
// Resource AWS::NetworkManager::SiteToSiteVpnAttachment Resource Type definition.
...
func (strct *Resource) MarshalJSON() ([]byte, error) {
	...
	comma = true
    // "VpnConnectionArn" field is required
    // only required object types supported for marshal checking (for now)
    // Marshal the "VpnConnectionArn" field
    if comma { 
        buf.WriteString(",") 
    }
    buf.WriteString("\"VpnConnectionArn\": ")
	if tmp, err := json.Marshal(strct.VpnConnectionArn); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *Resource) UnmarshalJSON(b []byte) error {
    ...
    VpnConnectionArnReceived := false
    var jsonMap map[string]json.RawMessage
    if err := json.Unmarshal(b, &jsonMap); err != nil {
        return err
    }
    ...
    for k, v := range jsonMap {
        ...
        case "VpnConnectionArn":
            if err := json.Unmarshal([]byte(v), &strct.VpnConnectionArn); err != nil {
                return err
             }
            VpnConnectionArnReceived = true
        default:
            return fmt.Errorf("additional property not allowed: \"" + k + "\"")
        }
    }
    ...
    // check if VpnConnectionArn (a required property) was received
    if !VpnConnectionArnReceived {
        return errors.New("\"VpnConnectionArn\" is required but was not present")
    }
    return nil
}
